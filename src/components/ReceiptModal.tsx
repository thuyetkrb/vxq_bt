...
export default function ReceiptModal({
  ...
  const handleSimulateDownload = () => {
    // Elegant simulation of CSV / receipt text summary download or SVG capture
    const boundary = "=========================================";
    const txt = `${boundary}
...
Học phí thu: ${formatVND(payment.amount)}
Trạng thái đóng: ĐÃ THANH TOÁN
Ngày thu phí: ${payment.paidDate || 'N/A'}
Người thu tiền: ${payment.collectedBy || 'N/A'}
Ghi chú: ${payment.note || 'Không có'}
${boundary}
Cảm ơn quý phụ huynh và võ sinh đã đồng hành
cùng võ đường WINGCHUN BINH TAN!
${boundary}`;
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-gray-100 bg-emerald-50 px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-emerald-900">Chi tiết biên lai học phí</span>
          </div>
          <button 
            onClick={onClose}
            className="rounded-full p-1.5 text-emerald-800 hover:bg-emerald-100 transition-colors"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="max-h-[75vh] overflow-y-auto p-6">
          <div 
            ...
            className="rounded-xl border border-dashed border-emerald-300 bg-emerald-50/20 p-6 font-sans text-gray-800 print:border-none print:bg-white print:p-0"
          >
            {/* Center Info */}
            <div className="text-center">
              <h2 className="text-sm font-bold tracking-wider text-emerald-800 uppercase print:text-black">{config.centerName}</h2>
            ...
            {/* Split Decorative */}
            <div className="my-4 border-b border-dashed border-emerald-200"></div>

            {/* Receipt Title */}
            <div className="text-center">
              <h1 className="text-lg font-bold text-emerald-900 uppercase tracking-wide print:text-black">Phiếu Thu Học Phí</h1>
              <p className="text-xs font-mono text-gray-400 mt-1 print:text-black">
                Số: <span className="text-emerald-700 font-bold print:text-black">{payment.receiptNo || 'MD2605XXX'}</span>
              </p>
            </div>
            ...
            <div className="mt-6 space-y-3 text-sm">
              ...
              <div className="flex flex-col gap-1 border-b border-gray-100 pb-1.5">
                <span className="text-gray-500">Ghi chú thu phí:</span>
                <span className="text-gray-700 italic text-xs">{payment.note || 'Đã kiểm đếm đầy đủ ngân quỹ'}</span>
              </div>
            </div>
            ...
            <div className="mt-8 text-center text-[11px] text-gray-400 italic">
              ...
              <p className="mt-1">Cảm ơn quý phụ huynh và võ sinh đã đồng hành cùng võ đường!</p>
            </div>
          </div>
        </div>
